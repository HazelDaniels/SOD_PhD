...
PCAS <- rda(PCASeeds, scale = FALSE)
barplot(as.vector(PCAS$CA$eig)/sum(PCAS$CA$eig))
sum((as.vector(PCAS$CA$eig)/sum(PCAS$CA$eig))[1:2])

plot(PCAS)
#plot(PCAS, display = "sites", type = "points")
#plot(PCAS, display = "species", type = "text")

#PCOA
sitePCAS <- PCAS$CA$u
speciesPCAS <- PCAS$CA$v

biplot(PCAS, choices = c(1,2), type = c("text", "points"), xlim = c(-5,10))
biplot(PCAS, choices = c(1,3), type = c("text","points"))

DistSeed <- vegdist(PCASeeds, method = "bray")
PCOAS <- pcoa(DistSeed)
barplot(PCOAS$values$Relative_eig[1:10])
biplot.pcoa(PCOAS, PCASeeds)

axesPCAS <- PCOAS$vectors[,c(1,2)]
...
NMDS1 <- metaMDS(DistSeed, k = 3, trymax = 100, trace = F)
NMDS1

Stress <- stressplot(NMDS1)
Stressplot <- plot(NMDS1, type = "t")

NMDS2 <- metaMDS(PCASeeds, k = 3, trymax = 100, trace = F)
NMDS2

NMDS3 <- metaMDS(PCASeeds, k = 3, trymax = 100, trace = F, autotransform = FALSE, distance="bray")
...
```{r}
#Interpretation
EFSites <- select(SODSites, 3, 8:11, 19, 23, 26)
...
```{r}
group = c(rep("Untreated", 56), rep("Treated", 44))
colors = c(rep("red", 56), rep("blue", 44))
